...
import type { SyncMediaOperation } from "../../../../interfaces/sync-media-operation.interface.js";
...
import mediaDiffLineFromMediaDiffAction from "../../../build/media-diff-line-from-media-diff-action.helper.js";
import mediaDiffActionsFromMediaDiffLines from "../../../build/media-diff-actions-from-media-diff-lines.helper.js";
import unlink, {
  type UnlinkError,
...
const build = {
  mediaDiffActionsFromMediaDiffLines,
  mediaDiffLineFromMediaDiffAction,
};
...
const syncConsoleMediaName = async (
  ...
  const [mediaDiffActions, parsingFailedLines] =
    build.mediaDiffActionsFromMediaDiffLines(diffLines);
  ...
  for (const mediaDiffAction of mediaDiffActions) {
    ...
    if (syncDiffActionError)
      failedDiffLines.push(
        build.mediaDiffLineFromMediaDiffAction(mediaDiffAction),
      );
  }
